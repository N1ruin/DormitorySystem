...
public enum SelectOrderRoomMenuItem {
    @MenuItem(itemAction = "Sort by number in ascending order",
            allowedRoles = {Role.SYSTEM_ADMIN, Role.UNIVERSITY_ADMIN, Role.DORMITORY_ADMIN})
    SORT_BY_NUMBER,
    @MenuItem(itemAction = "Sort by number in descending order",
            allowedRoles = {Role.SYSTEM_ADMIN, Role.UNIVERSITY_ADMIN, Role.DORMITORY_ADMIN})
    SORT_BY_NUMBER_DESC,
    @MenuItem(itemAction = "Sort by number of available seats in ascending order",
            allowedRoles = {Role.SYSTEM_ADMIN, Role.UNIVERSITY_ADMIN, Role.DORMITORY_ADMIN})
    SORT_BY_FREE_QUANTITY,
    @MenuItem(itemAction = "Sort by number of available seats in descending order",
            allowedRoles = {Role.SYSTEM_ADMIN, Role.UNIVERSITY_ADMIN, Role.DORMITORY_ADMIN})
